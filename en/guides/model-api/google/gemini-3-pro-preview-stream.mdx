...
description: "Call Google Gemini 3 Pro Preview streaming via Gemini API. Supports real-time SSE token streaming."
---

Gemini 3 Pro Preview (Stream) provides streaming text generation based on the Google Gemini 3 Pro Preview model, accessible through Cube AI. Responses are delivered in real time via Server-Sent Events (SSE).
...
<CodeGroup>
  ```bash cURL
  curl "https://ai.cubeuaeai.com/v1beta/models/gemini-3-pro-preview:streamGenerateContent?key=YOUR_API_KEY&alt=sse" \
  ...
  url = "https://ai.cubeuaeai.com/v1beta/models/gemini-3-pro-preview:streamGenerateContent"
...
<Card title="API Reference" icon="code" href="/en/api-reference/model-api/google/gemini-3-pro-preview-stream">
