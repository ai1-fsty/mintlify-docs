---
title: "Gemini 3.1 Flash Image Preview（流式）"
description: "通过 Gemini API 调用 Google Gemini 3.1 Flash Image Preview 进行流式图片生成。SSE 实时推送思考 chunk 与图片 chunk。"
---

Gemini 3.1 Flash Image Preview（流式）通过 Starrise AI 以原生 Gemini API 提供服务，支持实时 SSE 流式返回图片生成结果。思考 chunk 先行推送，最终图片 chunk 紧随其后。
...
<CodeGroup>
  ```bash cURL
  curl "https://ai.starrise.tech/v1beta/models/gemini-3.1-flash-image-preview:streamGenerateContent?key=YOUR_API_KEY" \
  ...
  url = "https://ai.starrise.tech/v1beta/models/gemini-3.1-flash-image-preview:streamGenerateContent"
...
<Card title="API 参考" icon="code" href="/zh/api-reference/model-api/google/gemini-3.1-flash-image-preview-stream">
