...
description: "通过 Cube AI API 调用 Black Forest Labs FLUX.1 Kontext [pro] 图片编辑与生成模型。"
---

FLUX.1 Kontext \[pro] 是 Black Forest Labs 推出的上下文感知图片生成模型，通过 Cube AI 提供。可以根据文字生成图片，也可以根据文字指令编辑已有图片。两个接口均为**同步调用** — 直接返回 Base64 编码的图片数据。
