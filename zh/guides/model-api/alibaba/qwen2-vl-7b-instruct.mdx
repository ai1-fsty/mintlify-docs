...
description: "通过 Starrise AI OpenAI 兼容接口调用Qwen2-VL-7B-Instruct 视觉模型（阿里巴巴渠道），快速、强大且易于集成。"
---

Qwen2-VL-7B-Instruct 是通过 Starrise AI 以 OpenAI 兼容接口提供的对话模型。
...
| 参数            | 类型             | 必填 | 说明                         |
| ------------- | -------------- | -- | -------------------------- |
| `model`       | string         | 是  | 固定为 `qwen2-vl-7b-instruct` |
...
<Card title="API 参考" icon="code" href="/zh/api-reference/model-api/alibaba/qwen2-vl-7b-instruct">
  查看 Qwen2-VL-7B-Instruct 的交互式 API Playground。
</Card>
