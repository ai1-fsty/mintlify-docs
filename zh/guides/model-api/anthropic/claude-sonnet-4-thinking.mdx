...
description: "Anthropic 的 Claude Sonnet 4 扩展思维版，以 Sonnet 级别的速度和成本实现增强推理。"
---

Claude Sonnet 4 Thinking 是 Claude Sonnet 4 的扩展思维版本，通过 Starrise AI 以 OpenAI 兼容接口提供服务。结合 Sonnet 在速度和能力之间的出色平衡，默认启用逐步内部推理，在复杂任务上显著提升准确率，同时保持快速响应。

## 核心能力

* **OpenAI 兼容** — 可直接替换 OpenAI SDK，无需修改其他代码
* **扩展思维** — 默认启用逐步内部推理
* **均衡性能** — 以 Sonnet 级别的速度和成本获得强推理能力
* **高级编程** — 擅长代码生成、调试和重构
* **长上下文** — 支持大文档处理和多轮对话
...
| 参数            | 类型             | 必填 | 说明                                      |
| ------------- | -------------- | -- | --------------------------------------- |
| `model`       | string         | 是  | 固定为 `claude-sonnet-4-20250514-thinking` |
...
<Card title="API 参考" icon="code" href="/zh/api-reference/introduction">
  查看 Claude Sonnet 4 Thinking 的交互式 API Playground。
</Card>
