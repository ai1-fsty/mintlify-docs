...
description: "通过 OpenAI 兼容接口调用 Anthropic 的 Claude 3.7 Sonnet，支持扩展思维的高级推理模型。"
---

Claude 3.7 Sonnet 是 Anthropic 最新的高性能对话模型，通过 Starrise AI 以 OpenAI 兼容接口提供服务。具备扩展思维（Extended Thinking）能力，擅长复杂编程、分析和多步骤推理任务。
...
| 参数            | 类型             | 必填 | 说明                       |
| ------------- | -------------- | -- | ------------------------ |
| `model`       | string         | 是  | 固定为 `claude-3-7-sonnet`  |
...
<Card title="API 参考" icon="code" href="/zh/api-reference/introduction">
  查看 Claude 3.7 Sonnet 的交互式 API Playground。
</Card>
