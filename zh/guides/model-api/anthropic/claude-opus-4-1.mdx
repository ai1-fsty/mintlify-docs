...
description: "通过 OpenAI 兼容接口调用 Anthropic 的 Claude Opus 4.1，最强大的 Claude 模型，适用于复杂推理和研究。"
---

Claude Opus 4.1 是 Anthropic 最强大、最智能的模型，通过 Starrise AI 以 OpenAI 兼容接口提供服务。在最高难度任务上提供顶尖表现，包括复杂研究、高级编程、数学推理和深度分析。
...
| 参数            | 类型             | 必填 | 说明                       |
| ------------- | -------------- | -- | ------------------------ |
| `model`       | string         | 是  | 固定为 `claude-opus-4-1`    |
...
<Card title="API 参考" icon="code" href="/zh/api-reference/introduction">
  查看 Claude Opus 4.1 的交互式 API Playground。
</Card>
